 ...
 * @param {object|function|undefined} [object.postData=undefined]
 * - object to be used for post
 * - assumed to be a callback function if callback is undefined
 * @param {function|undefined} [object.callback=undefined] function
 * @param {String|undefined} [object.version=undefiend] - To override the global version (Optional)
 */
...
function Graph({
  ...
  url = this.prepareUrl(url, version);
 ...
 */
Graph.prototype.prepareUrl = function (url, version) {
  url = this.cleanUrl(url);
  if (url.substr(0, 4) !== "http") {
    url = graphUrl + "/v" + (version ?? graphVersion) + url;
  }
...
Graph.prototype.get = function () {
  ...
  return request
    .get(this.options, function (err, res, body) {
      if (err) {
        self.callback(
          {
...
exports.get = function ({ url, params, version, callback }) {
  ...
  return new Graph({ method: "GET", url, callback, version });
};
/**
 ...
 * @param {object} object
 * @param {string} object.url
 * @param {object} object.params - It will be used as postData
 * @param {string} object.version
 * @param {function} object.callback
 */

exports.post = function ({ url, params, version, callback }) {
  if (typeof url !== "string") {
    return callback({ message: "Graph api url must be a string" }, null);
  }
  ...
  if (typeof params === "function") {
    callback = params;
    params =
      url.indexOf("access_token") !== -1 ? {} : { access_token: accessToken };
  }
 ...
 * @param {string} object.url
 * @param {object} object.params (optional) - It will be used as postData
 * @param {string} object.version
 * @param {function} object.callback
 */

exports.del = function ({ url, params, version, callback }) {
  ...
  return this.post({ url, params, version, callback });
};
 ...
 *
 * @param {object} object
 * @param {object} object.options (search options)
 * @param {string} object.version
 * @param {function} object.callback
 */

exports.search = function ({ options, version, callback }) {
  options = options || {};
  var url = "/search?" + qs.stringify(options);
  return this.get({ url, callback, version });
};

/**
 * Perform a batch query on the graph api
 * @param {object} object
 * @param  {Array} object.reqs     An array containing queries
 * @param {string} object.version
 * @param  {[Object]|Object} object.additionalData Additional data to send, e.g. attachments or the `include_headers` parameter.
 * @param  {Function} object.callback
 *
 * @see https://developers.facebook.com/docs/graph-api/making-multiple-requests
 */

exports.batch = function ({ reqs, version, additionalData, callback }) {
  if (!(reqs instanceof Array)) {
    return callback(
      { message: "Graph api batch requests must be an array" },
      null
    );
  }
...
exports.setVersion = function (version) {
  ...
  oauthDialogUrlMobile =
    "https://m.facebook.com/v" + version + "/dialog/oauth?"; // oldest version for auth
